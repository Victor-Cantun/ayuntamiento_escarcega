...
def footer(canvas, doc, nombre_izq, nombre_der):
    canvas.saveState()
    canvas.setTitle("Poliza de caja")
...
def encabezado_documento(logo_izq_path, logo_der_path, nombre_empresa, nombre_documento):
    ...
    data = [
        [logo_izq, titulo_centrado, logo_der]
    ]
...
def generate_cash_policy_pdf(request,policy,cash_concepts,concepts_predial,receipts,name_izq,name_der,download=False):    
    ...
    title_doc = f"Poliza de caja {fecha_formateada}"
    ...
    #Cuenta de banco 
    data.append([policy.income_account.account, policy.income_account.description, '{0} {1:,.2f}'.format(pesos, policy.amount_entered) ,""])
    #Espacio
    data.append(["","","",""])
    #Conceptos
    for it in cash_concepts:
    ...
    data.append(["","","",""])
    #Espacio
    data.append(["","Cuentas predial","",""])
    #Conceptos de predial
    for it in concepts_predial:
        amount = '{0} {1:,.2f}'.format(pesos,it.amount)
        data.append([
            it.concept.account_number,
            it.concept.name,
            "",
            amount,
        ])
    ...
    data.append(["","","",""])
    #cartera predial 
    data.append(["","Cartera predial",'{0} {1:,.2f}'.format(pesos,policy.predial_portfolio),""]) 
    #Rezago por predial
    data.append(["","Rezago por predial","",'{0} {1:,.2f}'.format(pesos,policy.predial_backlog)]) 
    #Espacio
    data.append(["","","",""])
    #SUMA (DEBE-HABER)
    data.append(["","Suma",'{0} {1:,.2f}'.format(pesos,policy.debit),'{0} {1:,.2f}'.format(pesos,policy.credit)])
    #Espacio
    data.append(["","","",""])
    #Espacio
    data.append(["","","DEL","AL"])     
